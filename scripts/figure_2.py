...
Plots results of hydrologic calibration experiments w/ and w/o screening of parameters
...
if __name__ == '__main__':
    ...
    # name of examples
    example_names_str = [ 'Hydrologic model\nw/o screening ($n_P = 9$)',         'Hydrologic model\nw/ screening ($n_P = 6$)' ]  # for plots
    example_names     = [ 'ostrich-gr4j-salmon_',                                'ostrich-gr4j-salmon_eee-' ]  # in folder names
    npara             = [  9,                                                    6  ]
    budgets           = [  [ 225, 450, 900 ],                                    [ 225, 450, 900  ] ]

    # names of metrics
    metric_names_str   = [ 'KGE$_\mathrm{Q}$', 'KGE$_\mathrm{Q}$' ]  # for plots
    metric_names       = [ 'kge',              'kge' ]               # in folder names
    metric_names_raven = [ 'DIAG_KLING_GUPTA', 'DIAG_KLING_GUPTA' ]  # in Raven diagnostics
                ...
                for ibudget in budgets[iiexample]:
                    ...
                    for imetric in metric_names[iiexample:iiexample+1]:
                        ...
                        for itrial in range(ntrials):
                            ...
                            f = open("example_2/"+iexample+""+ialgo+"_"+imetric+"_budget-"+str(ibudget)+"/trial_"+str(itrial+1)+"/OstModel0.txt", "r")
                            content = f.readlines()
                            ...
                            history['iter'] = [ int(cc.split()[0]) for cc in content ]
                            if iexample.startswith('ostrich-gr4j-salmon'):
        ...
        for iialgo,ialgo in enumerate(algorithm_names):
            ...
            for iibudget,ibudget in enumerate(budgets[iiexample]):
                ...
                if imetric in ['rmse','fx','pbias']:
                    ...
                    valign = 'bottom'
                    delta = 0.0
                if ibudget == 225 and iiexample == 1 and ialgo == 'sce':  # label too low
                    delta = 0.05
            ...
            # # -------------------------
            # # plot - para values over course of calibration (ONLY LARGEST BUDGET)
            # # -------------------------

            # for iibudget,ibudget in enumerate(budgets[iiexample][-1:]):

            #     ipara  = 1
            #     iiplot = 27+9*((ipara)//3)+(ipara)%3+1  + iialgo*3  +  iiexample*54
            #     pos_plot_1 = position(12, 9, iiplot, hspace=hspace, vspace=vspace)

            #     ipara  = 9
            #     iiplot = 27+9*((ipara)//3)+(ipara)%3+1  + iialgo*3  +  iiexample*54
            #     pos_plot_9 = position(12, 9, iiplot, hspace=hspace, vspace=vspace)



            #     for ipara in range(npara[iiexample]):

            #         iiplot = 27+9*((ipara)//3)+(ipara)%3+1  + iialgo*3  +  iiexample*54
            #         print("iiplot = ",iiplot)

            #         sub = fig.add_axes(position(12, 9, iiplot, hspace=hspace, vspace=vspace)+[0.0,0.00,0.0,0.0]) # [left, bottom, width, height]

            #         # parameter name
            #         sub.text( 0.05, 0.95, str2tex('$p_{'+str(ipara+1)+'}$',usetex=usetex),
            #                                 ha = 'left', va = 'top',
            #                                 transform=sub.transAxes,
            ...
            #             xvals = np.array(dict_results[ialgo][iexample][str(ibudget)][imetric]['hist']['trial_'+str(itrial+1)]['iter'])
            #             yvals = np.array(dict_results[ialgo][iexample][str(ibudget)][imetric]['hist']['trial_'+str(itrial+1)]['para'])[:,ipara]

            #             color = '0.7'
            #             alpha = 0.7
            #             zorder = 10
            #             sub.plot( xvals,yvals,
            #                   linewidth=lwidth, linestyle='-', color=color, alpha=alpha, zorder=zorder)
